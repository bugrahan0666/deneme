...
exports.run = async (client, message, args) => {
  ...
  var user = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[0]));
   ...
   if(user.hasPermission("ADMINISTRATOR")) return message.channel.sendEmbed(new Discord.RichEmbed().addField(`<a:dikkat:697499533516603545> Bilgi` , `<a:yukleniyor:741424786433114172> Yöneticileri Susturamazsın`).setColor("2e0101").setFooter(message.author.tag ,message.author.avatarURL).setTimestamp()).then(m => m.delete(5000));
    ...
    var muterole = message.guild.roles.get("744630202293813359")
    
     var ceza = message.guild.roles.get("744630200842584114")
        

    if(!muterole) return message.channel.sendEmbed(new Discord.RichEmbed().addField(`<a:dikkat:697499533516603545> Bilgi` , `<a:yukleniyor:741424786433114172> Ses Mute Rolü Yok`).setColor("2e0101").setFooter(message.author.tag ,message.author.avatarURL).setTimestamp()).then(m => m.delete(5000));
    var muteTime = args[1];
...
if(sayı>banlimiti && tarih-ilkbantarihi <=banaralıgı) {
  ...
  message.channel.sendEmbed(new Discord.RichEmbed().addField(`<a:dikkat:697499533516603545> Bilgi` , `<a:yukleniyor:741424786433114172> Bu Kullanıcıya 1 Hafta İçerisinde 4'den Fazla Ses Mutesi Atıldığı <@&692111658356834304> Rolü Verirdi.`).setColor("2e0101").setFooter(message.author.tag ,message.author.avatarURL).setTimestamp()).then(m => m.delete(20000));
  ...
  .setTimestamp()  
    ...
    let sChannel = message.guild.channels.get("744630292861419522")
    if(!sChannel) return
    sChannel.send(embed)
  },1000)
    },  ms(muteTime));
