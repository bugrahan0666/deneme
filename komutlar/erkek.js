...
exports.run = (client, message, args) => {
  ...
  const kayıtlı = message.guild.roles.find(r => r.id === erkek);
  ...
  if(!message.member.roles.array().filter(r => r.id === teyitci)[0]) {
    ...
    .setDescription(`_${member} adlı kullanıcı <@${message.author.id}> tarafından \`${tag} ${nick} | ${yas}\` ismiyle, <@&${erkek}> olarak kayıt edildi!_`)
    .setFooter(footer)
    .setTimestamp()
    .setColor("GOLD")
    log.send(embed)
      message.channel.send(new Discord.RichEmbed().setColor('GOLD').setTitle(`${sembol} ${sunucuadi} - Kayıt Sistemi`).setDescription(`_${member} adlı kullanıcı <@${message.author.id}> tarafından <@&${erkek}> olarak kayıt edildi!_ `).setTimestamp().setFooter(footer))
  }
